...
const GetPermission = async(req,res)=>{
    try{
        ...
        const resData = PermissionData.map((val)=>{
            const permission = val.permission===1?"":val.permission===2?"Cretae":val.permission===3?"Edit":"Delete"
            let obj = {
                _id:val._id,
    ...
    try{
        ...
        if(roleData[0]){
            ...
            roleData[0].permission.map((val)=>{
                const permission = val.permission===1?"":val.permission===2?"Create":val.permission===3?"Edit":"Delete"
